      ...
      <b-col cols="12" sm="12" md="8" lg="6" xl="4">
        <b-form @submit.prevent="onFormSubmit">
          <b-form-group id="input-group-userId" label="User Id:" label-for="userId">
            <b-form-input
              id="userId"
              v-model="$v.registerForm.userId.$model"
              :state="$v.registerForm.userId.$dirty ? !$v.registerForm.userId.$error : null"
              type="text"
          ...
          <b-form-group id="input-group-name" label="Name:" label-for="name">
            <b-form-input
              ...
              :state="$v.registerForm.name.$dirty ? !$v.registerForm.name.$error : null"
              type="text"
          ...
          <b-form-group id="input-group-re-password" label="Re Password:" label-for="repassword">
            <b-form-input
              id="repassword"
              v-model="$v.registerForm.repassword.$model"
              :state="$v.registerForm.repassword.$dirty ? !$v.registerForm.repassword.$error : null"
              type="password"
    ...
    onFormSubmit() {
      if (!this.$v.registerForm.$invalid) {
        this.register(this.registerForm)
          .then(resp => {
            if (resp) this.$router.replace("/login");
          })
          .catch(reason => {
            alert(reason);
          });
